	...
	Cell: {
		...
		getBlankedLength: function() {
			...
			for (var k = 0; k < addrs.length; k++) {
				...
				blanked += "" + (!cell.isnull && cell.isUnshade() ? 1 : 0);
			}
			var blanks = blanked.split(/0+/);
			if (blanks.length > 0) {
				if (blanks[0].length === 0) {
					blanks.shift();
				}
				if (blanks[blanks.length - 1].length === 0) {
					blanks.pop();
				}
				if (
					blanks.length > 1 &&
	...
	AnsCheck: {
		...
		checkCountOfClueCell: function() {
			this.checkAllCell(function(cell) {
				...
				var blanks = cell.getBlankedLength(); // 順番の考慮は不要
				if (cell.qnums.length !== blanks.length) {
					return true;
				}
				for (var i = 0; i < cell.qnums.length; i++) {
